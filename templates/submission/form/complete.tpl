...
<p>{translate key="plugins.themes.psychOpen.submission.submit.submissionCompleteThanks"} <em>{$context->getLocalizedName()}</em>.</p>

<div class="separator"></div>
<h3>{translate key="submission.submit.whatNext"}</h3>
<p>{translate key="plugins.themes.psychOpen.submission.submit.whatNext.description"}</p>
...
{* Additions for Personality Science *}
{if $currentContext->getId() == 11 }
    {translate key="plugins.themes.psychOpen.ps.feedback"}
{/if}   
